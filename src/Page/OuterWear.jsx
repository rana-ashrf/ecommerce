...
function OuterWear() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/outerwear")
      .then(res => setOuterwear(res.data))
  ...
  let filtered = outerwear.filter(item =>
    item.active !== false &&
        ...
        {filtered.length === 0 ? (
          ...
          <div className="product-grid">
            {filtered.map(item => {
              ...
              return (
                <div key={item.id} className="product-card" onClick={() => navigate(`/outerwear/${item.id}?${searchParams.toString()}`)}>
